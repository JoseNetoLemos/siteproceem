...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>PÁGINA DE EDIÇÃO DE CONTEÚDOS DE ATUALIDADES</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="../dashboardprofessores.php">Disciplinas</a></li>
        </ol>
        <button class="btn btn-dark"><a href="publicacoes/atualidades.php">VIZUALIZAR PUBLICAÇÕES</a></button>
      </nav>
    </div><!-- End Page Title -->
...
if (isset($_POST['tema'])){
    ...
    $texto = $_POST['texto'];
    $fk_atualidades = addslashes($_SESSION['id_usuario_professor']['id_usuario_professor']);

    if(!empty($nome) && !empty($email) && !empty($tema) && 
    !empty($texto) && !empty($fk_atualidades) ){

        $u -> conectar($db,$host,$user,$pass);
        
        if($u->msgErro == ""){

            if($u -> cadastraratualidades($nome, $email, $tema, $texto, $fk_atualidades)){
