...
if (isset($_POST['tema'])){
    ...
    $texto = $_POST['texto'];
    $fk_filosofia = addslashes($_SESSION['id_usuario_professor']['id_usuario_professor']);

    if(!empty($nome) && !empty($email) && !empty($tema) && 
    !empty($texto) && !empty($fk_filosofia) ){

        $u -> conectar($db,$host,$user,$pass);
        
        if($u->msgErro == ""){

            if($u -> cadastrarfilosofia($nome, $email, $tema, $texto, $fk_filosofia)){
