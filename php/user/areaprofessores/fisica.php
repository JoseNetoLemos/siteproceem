...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>PÁGINA DE EDIÇÃO DE CONTEÚDOS DE FÍSICA</h1>
      <nav>
        <ol class="breadcrumb">
        ...
        <button class="btn btn-dark"><a href="publicacoes/fisica.php">VIZUALIZAR PUBLICAÇÕES</a></button>
      </nav>
    </div><!-- End Page Title -->
...
if (isset($_POST['tema'])){
    ...
    $texto = $_POST['texto'];
    $fk_fisica = addslashes($_SESSION['id_usuario_professor']['id_usuario_professor']);

    if(!empty($nome) && !empty($email) && !empty($tema) && 
    !empty($texto) && !empty($fk_fisica) ){

        $u -> conectar($db,$host,$user,$pass);
        
        if($u->msgErro == ""){

            if($u -> cadastrarfisica($nome, $email, $tema, $texto, $fk_fisica)){
