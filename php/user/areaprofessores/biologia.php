...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>PÁGINA DE EDIÇÃO DE CONTEÚDOS DE BIOLOGIA</h1>
      <nav>
        <ol class="breadcrumb">
         ...
         <button class="btn btn-dark"><a href="publicacoes/biologia.php">VIZUALIZAR PUBLICAÇÕES</a></button>
      </nav>
    </div><!-- End Page Title -->
...
if (isset($_POST['tema'])){
    ...
    $texto = $_POST['texto'];
    $fk_biologia = addslashes($_SESSION['id_usuario_professor']['id_usuario_professor']);

    if(!empty($nome) && !empty($email) && !empty($tema) && 
    !empty($texto) && !empty($fk_biologia) ){

        $u -> conectar($db,$host,$user,$pass);
        
        if($u->msgErro == ""){

            if($u -> cadastrarbiologia($nome, $email, $tema, $texto, $fk_biologia)){
